...
$sql->FullRead("SELECT count(*) total FROM tb_sys003 C
                JOIN tb_sys004 EQ ON EQ.id_categoria = C.id
                JOIN tb_sys006 IE ON IE.patrimonio = EQ.patrimonio
                AND IE.status = :STS AND C.id = :CATEG ", "STATUS=5&CATEG={$categoria}");
$total = $sql->getRowCount()[0]['total'];
...
$sql->FullRead("SELECT C.descricao equipamento,count(*) total 
                    FROM tb_sys004 EQ
                        ...
                        JOIN tb_sys006 IE ON IE.patrimonio = EQ.patrimonio AND IE.status = :STS AND C.id = :CATEG group by C.id order by C.descricao","STS=5&CATEG={$categoria}");
...
$sql->FullRead("SELECT 
                ...
                FROM tb_sys004 EQ
                    ...
                    tb_sys008 L ON L.id = EQ.id_local
                        JOIN
                    tb_sys003 C ON C.id = EQ.id_categoria AND IE.status = A.id_status AND A.id_status = :STS AND C.id = :CATEG ORDER BY E.data","STS=5&CATEG={$categoria}");
        ...
        // Define o título da planilha 
        $objPHPExcel->getActiveSheet()->setTitle(strtoupper($equipamento));
        ...
        $sql->FullRead("SELECT P.id_peca codigo,
                        ...
                        FROM tb_sys015 P 
                            ...
                            JOIN tb_sys004 EQ ON EQ.patrimonio = IE.patrimonio AND EQ.id_categoria = :CATEG AND A.id_status = :STS group by P.id_peca order by quantidade desc","CATEG={$categoria}&STS=5");
