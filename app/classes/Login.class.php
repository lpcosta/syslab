...
class Login {
    ...
    private function getUser() {
        $senha = new Senha();
        $sql = new Read();      
        $sql->FullRead("SELECT id,nome,email,login,situacao,tentativa_login,senha_padrao,tipo,grupo_id FROM tb_sys001 WHERE login = :LOGIN AND senha = :SENHA", "LOGIN={$this->Login}&SENHA={$senha->setSenha($this->Senha)}");
